...
#include "nucleus/utils/terrain_mesh_index_generator.h"
 ...
 using gl_engine::AvalancheWarningLayer;
 ...
 void AvalancheWarningLayer::init()
 {
 ...
 const nucleus::tile_scheduler::DrawListGenerator::TileSet AvalancheWarningLayer::generate_tilelist(const nucleus::camera::Definition& camera) const
 {
     return m_draw_list_generator.generate_for(camera);
 }

 const nucleus::tile_scheduler::DrawListGenerator::TileSet AvalancheWarningLayer::cull(
     const nucleus::tile_scheduler::DrawListGenerator::TileSet& tileset, const nucleus::camera::Frustum& frustum) const
 {
     return m_draw_list_generator.cull(tileset, frustum);
 }

 void AvalancheWarningLayer::draw(ShaderProgram* shader_program, const nucleus::camera::Definition& camera,
     const nucleus::tile_scheduler::DrawListGenerator::TileSet& draw_tiles, bool sort_tiles, glm::dvec3 sort_position) const
 {
     ...
     shader_program->set_uniform("eaws_region_sampler", 2);
     shader_program->set_uniform("height_texture_layer_map_sampler", 3);
     shader_program->set_uniform("tile_id_map_sampler", 4);
     shader_program->set_uniform("height_sampler", 1); // Eaws stuff does not need this acutally
     ...
     for (const auto& tileset : tile_list) {
         ...
         height_texture_layer.emplace_back(tileset.second->eaws_texture_layer);
     }
 ...
 void AvalancheWarningLayer::remove_tile(const tile::Id& tile_id)
 {
 ...
 void AvalancheWarningLayer::initilise_attribute_locations(ShaderProgram* program)
 {
...
void AvalancheWarningLayer::set_aabb_decorator(const nucleus::tile_scheduler::utils::AabbDecoratorPtr& new_aabb_decorator)
{
    m_draw_list_generator.set_aabb_decorator(new_aabb_decorator);
}

void AvalancheWarningLayer::set_quad_limit(unsigned int new_limit)
{
    m_loaded_tiles.resize(new_limit * 4);
    std::fill(m_loaded_tiles.begin(), m_loaded_tiles.end(), tile::Id { unsigned(-1), {} });
}

void AvalancheWarningLayer::add_tile(const tile::Id& id, tile::SrsAndHeightBounds bounds, const nucleus::Raster<uint16_t>& eaws_raster)
{
    if (!QOpenGLContext::currentContext()) // can happen during shutdown.
        return;
    ...
    *t = id;
    ...
    m_eaws_textures->upload(eaws_raster, layer_index);
...
void AvalancheWarningLayer::update_gpu_id_map()
{
    ...
    for (const auto tile : m_gpu_tiles) {
        ...
        texture_ids.pixel(hash_to_pixel(hash)) = tile.eaws_texture_layer;
    }
    m_texture_id_map_texture->upload(texture_ids);
    m_tile_id_map_texture->upload(packed_ids);
}

void AvalancheWarningLayer::set_permissible_screen_space_error(float new_permissible_screen_space_error)
{
    m_draw_list_generator.set_permissible_screen_space_error(new_permissible_screen_space_error);
}

void AvalancheWarningLayer::update_gpu_quads(
    const std::vector<nucleus::tile_scheduler::tile_types::GpuEawsQuad>& new_quads, const std::vector<tile::Id>& deleted_quads)
{
    ...
    for (const auto& quad : new_quads) {
        for (const auto& tile : quad.tiles) {
            ...
            assert(tile.raster);
            add_tile(tile.id, tile.bounds, *tile.raster);
        }
    }
    update_gpu_id_map();
}
